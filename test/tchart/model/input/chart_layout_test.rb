...
module TChart
  describe ChartLayout, "items_date_range" do
    ...
    it "returns a range from the earliest chart item start date to the latest chart item end date" do
      ...
      ChartLayout.new(@settings, [item1, item2]).items_date_range.must_equal Date.new(2000, 11, 1)..Date.new(2008, 3, 30)
    end
    it "returns 1st January to 31st December when chart items is empty" do
      this_year = Date.today.year
      ChartLayout.new(@settings, []).items_date_range.must_equal Date.new(this_year,1,1)..Date.new(this_year,12,31)
    end
    it "returns 1st January to 31st December when none of the chart items have date ranges" do
      ...
      this_year = Date.today.year
      ChartLayout.new(@settings, [item1, item2]).items_date_range.must_equal Date.new(this_year,1,1)..Date.new(this_year,12,31)
    end
  end

  describe ChartLayout, "x_item_dates" do
    before do
      settings = stub
      @layout = ChartLayout.new(settings, [])
    end
  ...
  describe ChartLayout, "x_item_x_coordinates" do
    before do
      @settings = stub
      @layout = ChartLayout.new(@settings, [])
    end
  ...
  describe ChartLayout, "x_axis_length" do
    before do
      @settings = stub(chart_width: 130, x_item_label_width: 10, y_item_label_width: 20)
      items = stub
      @layout = ChartLayout.new(@settings, items)
    end
  ...
  describe ChartLayout, "y_axis_length" do
    before do
  ...
  describe ChartLayout, "y_item_x_coordinate" do
    before do
  ...
  describe ChartLayout, "y_item_y_coordinates" do
    before do
      settings = stub(line_height: 10)
      items = [ stub, stub, stub ]
      @layout = ChartLayout.new(settings, items)
  ...
  describe ChartLayout, "date_range_to_x_coordinates" do
    before do

...
module TChart
  ...
  describe Layout, "x_axis_tick_dates" do
    before do
      settings = stub
  ...
  describe Layout, "x_axis_tick_x_coordinates" do
    before do
      @settings = stub
      @layout = Layout.new(@settings, [])
    end
    it "returns an array of x coordinates" do
      @layout.stubs(:x_axis_tick_dates).returns (2000..2005).step(1)
  ...
  describe Layout, "x_axis_length" do
    before do
      @settings = stub(chart_width: 130, x_axis_label_width: 10, y_axis_label_width: 20)
      items = stub
  ...
  describe Layout, "y_axis_label_x_coordinate" do
    before do
      @settings = stub(x_axis_label_width: 10, y_axis_label_width: 20)
      items = stub
      @layout = Layout.new(@settings, items)
    end
  ...
  describe Layout, "y_axis_tick_y_coordinates" do
    ...
    it "returns the y coordinates of all items" do
      @layout.y_axis_tick_y_coordinates.to_a.must_equal [30, 20, 10]
    end
  end
  
  describe Layout, "date_range_to_x_coordinates" do
    before do
      settings = stub( chart_width: 130, x_axis_label_width: 10, y_axis_label_width: 20, line_height: 10 )
      items = stub
      @layout = Layout.new(settings, items)

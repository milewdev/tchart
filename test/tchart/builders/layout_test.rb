...
module TChart
  describe Layout, "y_items_date_range" do
    before do
      @settings = stub
    end
    it "returns a range from the earliest chart item start date to the latest chart item end date" do
      ...
      Layout.new(@settings, [item1, item2]).y_items_date_range.must_equal Date.new(2000, 11, 1)..Date.new(2008, 3, 30)
    end
    it "returns 1st January to 31st December when chart items is empty" do
      this_year = Date.today.year
      Layout.new(@settings, []).y_items_date_range.must_equal Date.new(this_year,1,1)..Date.new(this_year,12,31)
    end
    it "returns 1st January to 31st December when none of the chart items have date ranges" do
      ...
      this_year = Date.today.year
      Layout.new(@settings, [item1, item2]).y_items_date_range.must_equal Date.new(this_year,1,1)..Date.new(this_year,12,31)
    end
  end
  ...
  describe Layout, "x_item_dates" do
    ...
    it "returns the correct dates when the items date range is less than 10 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2004,10,4)
    ...
    it "returns the correct dates when the items date range is 10 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2009,10,4)
    ...
    it "returns the correct dates when the items date range is 11 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2010,10,4)
    ...
    it "returns the correct dates when the items date range is less than 50 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2044,10,4)
    ...
    it "returns the correct dates when the items date range is 50 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2049,10,4)
    ...
    it "returns the correct dates when the items date range is less than 60 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2054,10,4)
    ...
    it "returns the correct dates when the items date range is 60 years" do
      @layout.stubs(:y_items_date_range).returns Date.new(2000,3,17)..Date.new(2059,10,4)
      @layout.x_item_dates.must_equal (2000..2060).step(10).to_a
    end
  end

...
module TChart  
  describe DateLabelsBuilder, "build" do
    before do
      x_length = 100
      chart_items = [ stub(:date_ranges => [ Date.new(2000,3,17)..Date.new(2003,10,4) ]) ]
      @date_labels = DateLabelsBuilder.build(chart_items, x_length)
    end
    it "builds the correct number of labels" do
      @date_labels.length.must_equal 5
    end
    it "builds labels with the correct text" do
      @date_labels[0].date.must_equal Date.new(2000,1,1)
      @date_labels[1].date.must_equal Date.new(2001,1,1)
      @date_labels[2].date.must_equal Date.new(2002,1,1)
      @date_labels[3].date.must_equal Date.new(2003,1,1)
      @date_labels[4].date.must_equal Date.new(2004,1,1)
    end
    it "builds labels with the correct x-coordinate" do
      @date_labels[0].x_coordinate.must_equal 0
      @date_labels[1].x_coordinate.must_equal 25
      @date_labels[2].x_coordinate.must_equal 50
      @date_labels[3].x_coordinate.must_equal 75
      @date_labels[4].x_coordinate.must_equal 100
    end
  end

  describe DateLabelsBuilder, "find_chart_items_date_range" do
    it "returns a range from the earliest chart item start date to the latest chart item end date" do
      ...
      chart_items_date_range = DateLabelsBuilder.find_chart_items_date_range( [chart_item1, chart_item2] )
    ...
    it "returns 1st January to 31st December when chart items is empty" do
      chart_items_date_range = DateLabelsBuilder.find_chart_items_date_range( [] )
  ...
  describe DateLabelsBuilder, "calc_date_range_and_date_interval" do
    it "calculates the correct range and interval when the chart items date range is less than 10 years" do
      ...
      date_range, date_interval = DateLabelsBuilder.calc_date_range_and_date_interval(chart_items_date_range)
  ...
  describe DateLabelsBuilder, "calc_number_of_labels" do
    it "calculates the correct number of labels" do
      date_range = Date.new(2000,1,1)..Date.new(2010,1,1)
      date_interval = 5
      number_of_labels = DateLabelsBuilder.calc_number_of_labels(date_range, date_interval)
      number_of_labels.must_equal 3
    end
  end
  
  describe DateLabelsBuilder, "calc_x_coordinate_interval" do
    it "calculates the correct x-coordinate interval between labels" do
      x_length = 100
      number_of_labels = 11
      x_coordinate_interval = DateLabelsBuilder.calc_x_coordinate_interval(x_length, number_of_labels)

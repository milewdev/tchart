module TChart
  module XLabelsBuilder
    def self.build(settings, chart_items, x_length) # => [ XLabel, ... ]
      ...
      dates.zip(x_coordinates).map { |date, x_coordinate| XLabel.new(date, x_coordinate, settings.x_label_y_coordinate) }
    end
